# scenario-J-maxrounds-live — spawn 1 (full, untruncated)
...
```
You are REVIEWER 2 (structure) in a dual-review loop. Your rubric is the `dual-review-structure` skill.
...
- and every file under /home/liam/git/Arbor/plugins/dual-review-loop/skills/dual-review-structure/references/
...
--- Full current diff vs baseline (git diff HEAD) ---
diff --git a/store.py b/store.py
index c87bfe3..a638e09 100644
--- a/store.py
+++ b/store.py
...
index 33a2c46..69c6581 100644
--- a/test_store.py
+++ b/test_store.py
@@ -14,6 +14,23 @@ class TestStore(unittest.TestCase):
...
Rules that apply to you (structure reviewer):
- `verdict: PASS` with an empty findings list is a valid, expected outcome.
- Set `blocking: true` ONLY for findings whose `discipline:` is `blocking regression`.
- `discipline:` is MANDATORY on every finding you raise — `blocking regression` | `material improvement` | `taste`.
- A `taste` observation must NOT be reported as a finding; at most mention it in one aggregate `residual_risks` line.
