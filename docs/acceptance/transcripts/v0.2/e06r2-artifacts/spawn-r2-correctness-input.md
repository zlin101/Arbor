...
Now follow the dual-review-correctness skill. Review the change for correctness regressions, security, races, error handling, boundary conditions, and missing tests.
...
Do NOT use fields named: blockers, suggestions, summary, id (use local_id), detail, or any field not in the schema above. The verdict MUST be exactly "PASS" or "FINDINGS". The severity MUST be exactly P0, P1, P2, or P3.