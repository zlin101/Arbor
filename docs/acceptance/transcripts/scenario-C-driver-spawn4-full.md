# scenario-C-driver — spawn 4 (full, untruncated)

- description: Round 2 structure reviewer
- subagent_type: general-purpose

## Full prompt

```
You are the STRUCTURE REVIEWER in a dual-review loop. You never fix and never write.
...
2. Every file in /home/liam/git/Arbor/plugins/dual-review-loop/skills/dual-review-structure/references/ (structural-quality-checklist.md)
...
Everything below is the FULL current change (baseline → current working tree). This is NOT a fix-only diff: review the whole change against the frozen baseline.
...
+
+def _is_sys(key):
+    """True if key names a protected system key.
+
+    Only string keys can be system keys, so non-string keys are ordinary keys.
+    """
+    return isinstance(key, str) and key.startswith(SYS_PREFIX)
+
 
 class Store:
+    """In-memory key-value store with an audit log.
+
+    Keys starting with ``SYS_PREFIX`` are read-only system keys: ``set`` refuses
+    them, ``get`` reports ``PROTECTED`` for them (in place of ``default``), and
+    ``delete`` ignores them.
+    """
+
     def __init__(self):
...
Rules: `verdict: PASS` with an empty findings list is valid. As the structure reviewer, set `blocking: true` ONLY for `discipline: blocking regression`; `discipline:` is mandatory on every structure finding, and `taste` findings must NOT be reported as findings (at most one aggregate line in `residual_risks`). Do not generate global ids, do not merge with another reviewer's findings, do not compare notes. Return the YAML envelope as your final message.
```
