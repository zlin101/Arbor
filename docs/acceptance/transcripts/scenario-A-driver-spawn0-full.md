# scenario-A-driver — spawn 0 (full, untruncated)
...
```
You are the CORRECTNESS reviewer in a dual-review loop. You are a worker, not a writer.
...
- plus every file under /home/liam/git/Arbor/plugins/dual-review-loop/skills/dual-review-correctness/references/
...
Rules: `verdict: PASS` with an empty findings list is valid. Set `blocking: true` for P0/P1 (and a P2 only when it directly endangers the change's correctness). Do not generate global ids, do not merge others' findings, do not compare notes.
```
