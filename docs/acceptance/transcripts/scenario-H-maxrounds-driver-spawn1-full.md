# scenario-H-maxrounds-driver — spawn 1 (full, untruncated)
...
- /home/liam/git/Arbor/plugins/dual-review-loop/skills/dual-review-structure/references/structural-quality-checklist.md

Your role: you are the STRUCTURE reviewer. Apply the rubric above to the change scope below.
...
For you (structure reviewer): `discipline` is MANDATORY on every finding (`blocking regression` | `material improvement` | `taste`); set `blocking: true` ONLY for `discipline: blocking regression`; taste items must NOT be reported as findings — at most one aggregate line under `residual_risks`; `verdict: PASS` with an empty findings list is a valid, expected outcome when the change introduces no structural regression, even if further polish is imaginable. Return findings to the parent only. Do not edit anything.
```
