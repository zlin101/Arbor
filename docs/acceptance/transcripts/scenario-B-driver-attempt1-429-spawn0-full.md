# scenario-B-driver-attempt1-429 — spawn 0 (full, untruncated)
...
```
You are Reviewer A1 (correctness) in a dual-review loop for the repo at /tmp/arbor-acceptance/fixture-b.
...
- and every file under /home/liam/git/Arbor/plugins/dual-review-loop/skills/dual-review-correctness/references/ (correctness-checklist.md, security-reliability-checklist.md)
...
```
Rules: `verdict: PASS` with an empty findings list is valid and expected. Set `blocking: true` for P0/P1 (and for a P2 only when it directly endangers the change's correctness). Do not generate global ids, do not merge with the other reviewer, do not compare notes. Do not write or edit any file.
```
