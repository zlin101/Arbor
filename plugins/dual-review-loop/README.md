...
| `dual-review-structure` | 只读 reviewer：结构简化 / 抽象质量 / spaghetti 增长 / 层与类型边界，blocking-vs-taste 纪律 |

## 用法

对当前工作区、branch、commit range 或显式文件列表运行循环；用户可显式指定
`strict`（P2 也作为收敛门）与 `max_rounds` 覆盖。其余全部走默认合同
（`max_rounds: 3`，连续 2 轮无进展即停）。
